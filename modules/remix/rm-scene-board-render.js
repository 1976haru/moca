/* ================================================
   modules/remix/rm-scene-board-render.js
   영상 리믹스 스튜디오 — Scene 보드 렌더링 (HTML 생성)

   * rm-scene-board.js 가 1000+ 줄을 넘어 분리.
     기존 동작은 그대로. 액션 핸들러는 rm-scene-board-actions.js 에 있음.
   * 이 파일은 순수 렌더 only — 상태를 변경하지 않고 HTML 문자열만 반환.
   * window._rmBoardRender 로 노출. rm-scene-board.js (wrapper) 가
     RM_BOARD.render 로 다시 export.
   ================================================ */
(function(){
  'use strict';
  ...
  /* ── 서버 자동 가져오기 모드 — 실제 렌더는 rm-server-ui.js 가 담당 ── */
